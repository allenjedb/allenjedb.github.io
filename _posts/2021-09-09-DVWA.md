...
<p>In this section we will see a page where we can input a user id. <img src="/assets/images/dvwa/SQLi/1.png" alt="" /><br>Let's try user id 1.<br><img src="/assets/images/dvwa/SQLi/2.png" alt="" /><br>User Id 1 gives us a result of admin. As from our earlier sections we know that we have 5 users. Admin, 1337, smithy, pablo and gordonb. The goal of this section is to use SQL injection to get their passwords. Now let's try sending a single quote to our input box.<img src="/assets/images/dvwa/SQLi/3.png" alt="" /><br>This gives us an error that probably tells us that the inputs to this application is getting executed as SQL statements on the database. If click the view source option in DVWA this will show us the query that's getting executed.<img src="/assets/images/dvwa/SQLi/4.png" alt="" /><br><i>"SELECT first_name, last_name FROM users WHERE user_id = 'id'";</i><br>So if we try another basic SQL or query like <i>a' or'1'='1</i> this will result to <i>"SELECT first_name, last_name FROM users WHERE user_id = 'a' or'1'='1'";</i> This query being executed.