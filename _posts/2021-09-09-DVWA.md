...
<p>Now we can try changing the URL and we will pretend to be a victim, that we're going to send the malicious link to. Let's change the link to <i>192.168.56.103/vulnerabilities/csrf/?password_new=csrf&password_conf=csrf&Change=Change#</i> this will change the victims password to <i>csrf</i> So let's open a new tab paste and then enter the malicious link <img src="/assets/images/dvwa/CSRF/5.png" alt="" /><br>This will land us back to the CSRF section and it will confirm that the password has been changed.<img src="/assets/images/dvwa/CSRF/6.png" alt="" /><br></p>

<p>Now if we try to login again to our admin user and enter csrf it will let us log in.</p>